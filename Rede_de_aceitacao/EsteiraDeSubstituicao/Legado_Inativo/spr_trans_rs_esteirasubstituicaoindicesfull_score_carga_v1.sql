...
;
insert into #IndiceDefalhas (protocolo, conveniadoid, conveniadopassagemid, datapassagem, datatransacao, codigopraca, codigopista, obuid, indicefalha)
select  *
...
drop table if exists #CalculoScoreSubstituicao; --mudar hardcoded para a tabela referencia quando souber o nome da mesma
Select *, 
Case 
when indicefalha = 0 and protocolo = 'Artesp' then '-1' -- when indicefalha = 0 and protocolo = 'Artesp' then (select max(pontuacaofalhaartesp) from tabelaparametro) 
when indicefalha = 0 and protocolo = 'EDI' then '-1'    -- when indicefalha = 0 and protocolo = 'EDI' then (select max(pontuacaofalhaedi) from tabelaparametro) 
when indicefalha = 1 and protocolo = 'Artesp' then '2'  -- when indicefalha = 1 and protocolo = 'Artesp' then (select max(pontuacaosucessoartesp) from tabelaparametro) 
when indicefalha = 1 and protocolo = 'EDI' then '1'     -- when indicefalha = 1 and protocolo = 'EDI' then (select max(pontuacaosucessoedi) from tabelaparametro)
Else 0 end Score
into #CalculoScoreSubstituicao

from #IndiceDefalhas 
where datatransacao >= cast (getdate() -1 as Varchar (10))
...
inner join #CalculoScoreSubstituicao  B on a.obuid = b.obuid and cast (a.DataOrigemTransacoes  as Varchar (10)) = cast (b.datatransacao as Varchar (10))
);
...
getdate() DataScore, 
obuid, 
cast (datatransacao as Varchar (10)) as DataOrigemTransacoes,
Sum (Score) scorereal,  
case when Sum (Score) < 0 then 0 else Sum (Score) end  as Score

from #CalculoScoreSubstituicao 
group by obuid,cast (datatransacao as Varchar (10));
